...
int tq_init_queue_all(int mode)
{
	int i;
...
/* Create and add msg into queue */
int tq_create_add_msg(int type, char *str, int queue_index)
{
	timer_queue_msg tqm;
	char *ptr;
	int len;
        ...
        if (tq_get_msg_num(queue_index) != TIMER_QUEUE_MSG_NUM_MAX)
        {
                ...
                {
                        if (tq_get_msg_on_str(TIMER_QUEUE_MSG_TYPE_IPV4,
                                                str,
                                                queue_index) != NULL)
                                return 0;
			else
			{
				len = TIMER_QUEUE_IPV4_STRING_LEN;
				ptr = tqm.ipv4;
			}
                }
                else if (type == TIMER_QUEUE_MSG_TYPE_MAC)
                {
                        if (tq_get_msg_on_str(TIMER_QUEUE_MSG_TYPE_MAC,
                                                str,
                                                queue_index) != NULL)
                                return 0;
			else
			{
				len = TIMER_QUEUE_MAC_STRING_LEN;
				ptr = tqm.mac;
			}
                }
		else
		...
		/* Create the timer queue msg */
		tqm.type = type;
		memcpy(ptr, str, len);
		tqm.timer = time(NULL) + TIMER_THREAD_BLACKLIST_BLOCK_TIME;

		/* Save the msg into queue */
